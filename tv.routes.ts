...
export async function listMatchesHandler(req: Request, res: Response) {
  ...
  res.json(await matchesService.listMatches(req.auth.patotaId));
}
...
export async function createMatchHandler(req: Request, res: Response) {
  if (!req.auth) throw new ApiError(401, "Não autenticado");
  res.status(201).json(await matchesService.createMatch(req.body, req.auth.patotaId));
}
...
export async function setGuestPaidHandler(req: Request, res: Response) {
  if (!req.auth) throw new ApiError(401, "Não autenticado");
  res.json(await matchesService.setGuestPaid(req.params.id, req.params.guestId, req.body.paid !== false, req.auth.patotaId));
}
