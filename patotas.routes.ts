...
export async function ensureAttendanceForOpenMatches(playerId: string, patotaId: string) {
  const openMatches = await prisma.match.findMany({ where: { closed: false, patotaId } });
...
export async function listPlayers(patotaId: string, status?: "ATIVO" | "INATIVO") {
  return prisma.player.findMany({
    where: { patotaId, ...(status ? { status } : {}) },
...
export async function createPlayer(input: CreatePlayerInput, patotaId: string) {
...
export async function createLoginForPlayer(id: string, email: string, password: string, patotaId: string) {
  ...
  await prisma.user.create({
    data: { email, passwordHash, role: "JOGADOR", playerId: id, patotaId },
  });
...
export async function updatePlayerStatus(id: string, status: "ATIVO" | "INATIVO", patotaId: string) {
  const player = await getPlayerById(id);
  ...
  if (status === "ATIVO" && player.status === "INATIVO") {
    await ensureAttendanceForOpenMatches(id, patotaId);
  }
