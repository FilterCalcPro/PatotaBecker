...
export async function listTransactions(patotaId: string) {
  return prisma.transaction.findMany({ where: { patotaId }, orderBy: { date: "desc" } });
}

export async function createTransaction(input: CreateTransactionInput, patotaId: string) {
...
export async function getSummary(patotaId: string) {
  const [income, expense] = await Promise.all([
    prisma.transaction.aggregate({ where: { type: "ENTRADA", patotaId }, _sum: { amount: true } }),
    prisma.transaction.aggregate({ where: { type: "SAIDA", patotaId }, _sum: { amount: true } }),
  ]);
...
export async function listMonthlyFees(patotaId: string, month?: string) {
  return prisma.monthlyFee.findMany({
    where: { player: { patotaId }, ...(month ? { referenceMonth: month } : {}) },
    include: { player: true },
    orderBy: [{ referenceMonth: "desc" }, { player: { name: "asc" } }],
  });
}

export async function payMonthlyFee(id: string, input: PayFeeInput, patotaId: string) {
