...
export async function listPlayersHandler(req: Request, res: Response) {
  ...
  const players = await playersService.listPlayers(req.auth.patotaId, status);
  res.json(players);
}
...
export async function createPlayerHandler(req: Request, res: Response) {
  if (!req.auth) throw new ApiError(401, "Não autenticado");
  const player = await playersService.createPlayer(req.body, req.auth.patotaId);
...
export async function createLoginHandler(req: Request, res: Response) {
  if (!req.auth) throw new ApiError(401, "Não autenticado");
  const player = await playersService.createLoginForPlayer(req.params.id, req.body.email, req.body.password, req.auth.patotaId);
  res.status(201).json(player);
}
...
export async function updatePlayerStatusHandler(req: Request, res: Response) {
  if (!req.auth) throw new ApiError(401, "Não autenticado");
  const player = await playersService.updatePlayerStatus(req.params.id, req.body.status, req.auth.patotaId);
  res.json(player);
}
